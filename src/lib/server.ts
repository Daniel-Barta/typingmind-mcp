...
export async function start(
  authToken: string,
): Promise<{ port: number; host: string; protocol: "http" | "https" }> {
  const app = express();
  ...
  app.post("/start", auth, async (req: Request, res: Response) => {
    try {
      const { mcpServers } = req.body as {
        mcpServers: Record<string, ClientConfig>;
      };
  ...
  app.post("/restart/:id", auth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to restart client", details: error.message });
    }
  });
  ...
  app.get("/clients/:id/tools", auth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to get tools", details: error.message });
    }
  });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to call tool", details: error.message });
      }
    },
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    let server:
      | (https.Server & Closable)
      | (ReturnType<typeof app.listen> & Closable);
